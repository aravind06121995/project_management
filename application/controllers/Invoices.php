...
class Invoices extends CI_Controller 
{
	...
	public function add_check() 
	{
		...
		// Custom Fields
		$fields = $this->invoices_model->get_custom_fields();
		$user_data = array();
		foreach($fields->result() as $r) {
			if(isset($data["field_id_" . $r->ID])) {
				$value = $this->common->nohtml($data["field_id_" . $r->ID]);
				if(!empty($value)) {
					$user_data[] = array("id" => $r->ID, "data" => $value);
				} else {
					if($r->required) {
						$field_errors['field_id_' . $r->ID] = lang("error_210");
					}
				}
			} else {
				if($r->required) {
	...
	public function index($projectid = 0) 
	{
		...
		if($this->common->has_permissions(
			...
			$projects = $this->projects_model
				->get_projects_user_all_no_pagination($this->user->info->ID, "(pm2.admin = 1 OR pm2.invoice = 1)");
		}
	...
	public function estimates($projectid = 0) 
	{
		$this->check_requirements(array("admin", "project_admin",
			 "invoice_manage", "invoice_worker"));
		$this->template->loadData("activeLink", 
			array("invoice" => array("estimates" => 1)));
		...
		if($projectid > 0) {
			...
			$this->common->check_permissions(
				"You can't view Estimates for this project!", 
				array("admin", "project_admin", "invoice_manage"), // User Roles
				array("admin", "invoice"),  // Team Roles
				$projectid
			);
		...
		$this->template->loadContent("invoices/estimates.php", array(
			"page" => "index",
			"projects" => $projects,
			"project" => $project
			)
	...
	public function estimate_page($page="index", $userid=0, $projectid=0) 
	{
		$userid = intval($userid);
		$projectid = intval($projectid);
		if($projectid > 0) {
		...
		if($page == "index") {
			...
			if($this->common->has_permissions(array("admin", "project_admin", "invoice_manage"), $this->user)) {
				$this->datatables->set_total_rows(
					$this->invoices_model->get_estimates_total($projectid)
				);

				$invoices = $this->invoices_model->get_estimates($projectid, $this->datatables);
			} else {
				$this->datatables->set_total_rows(
					$this->invoices_model->get_estimates_user_total($projectid, $this->user->info->ID)
				);

				$invoices = $this->invoices_model->get_estimates_user($projectid, $this->user->info->ID, $this->datatables);
			}
		} elseif($page == "client") {
			$this->check_requirements(array("admin", "project_admin", 
				"invoice_manage", "invoice_client"));
			$invoices = $this->invoices_model->get_estimates_client(
				$this->user->info->ID, $this->datatables);

			$this->datatables->set_total_rows(
				$this->invoices_model
					->get_estimates_client_total($this->user->info->ID)
			);
		}


		foreach($invoices->result() as $r) {
			if($r->estimate_status == 0) {
				$status = "<label class='label label-default'>".lang("ctn_1611")."</label>";
			} elseif($r->estimate_status == 1) {
				$status = "<label class='label label-success'>".lang("ctn_1612")."</label>";
			} elseif($r->estimate_status == 2) {
				$status = "<label class='label label-danger'>".lang("ctn_1613")."</label>";
			}

			$options = '';
			
			if($this->common->has_permissions(array("admin", "project_admin", "invoice_manage"), $this->user)) {
				$options .= '<a href="'.site_url("invoices/convert_invoice/" . $r->ID).'" class="btn btn-info btn-xs" title="'.lang("ctn_55").'"  data-toggle="tooltip" data-placement="bottom">'.lang("ctn_1614").'</span></a> <a href="'.site_url("invoices/edit_invoice/" . $r->ID).'" class="btn btn-warning btn-xs" title="'.lang("ctn_55").'"  data-toggle="tooltip" data-placement="bottom"><span class="glyphicon glyphicon-cog"></span></a> <a href="'.site_url("invoices/delete_invoice/" . $r->ID . "/" . $this->security->get_csrf_hash()).'" class="btn btn-danger btn-xs"  data-toggle="tooltip" data-placement="bottom" onclick="return confirm(\''.lang("ctn_317").'\')" title="'.lang("ctn_57").'"><span class="glyphicon glyphicon-trash"></span></a>';
			}
	...
	public function convert_invoice($id) 
	{
		...
		if($invoice->clientid > 0) {
			$this->user_model->increment_field($invoice->clientid, "noti_count", 1);
			$this->user_model->add_notification(array(
				"userid" => $invoice->clientid,
				"url" => "invoices/view/" . $id . "/" . $invoice->hash,
				"timestamp" => time(),
				...
				"email_notification" => $invoice->client_email_notification
				)
			);
		} elseif(!empty($invoice->guest_email)) {
			// send email
			...
			$this->common->send_email($email_template->title,
				 $email_template->message, $invoice->guest_email);
		}
	...
	public function add_pro() 
	{
		...
		$estimate = intval($this->input->post("estimate"));
		...
		foreach($user_data as $d) {
			$this->invoices_model->add_custom_field_data(array(
				"invoiceid" => $invoiceid,
				"fieldid" => $d['id'],
				"value" => $d['data']
				)
		...
		if($estimate) {
			redirect(site_url("invoices/estimates"));
		} else {
			if($this->common->has_permissions(array("admin", "project_admin", "invoice_manage"), $this->user)) {
				redirect(site_url("invoices"));
			} else {
	...
	public function edit_invoice($id) 
	{
		...
		$fields = $this->invoices_model->get_custom_field_invoice_data($invoice->ID);
	...
	public function edit_invoice_pro($id) 
	{
		...
		$this->invoices_model->delete_invoice_custom_data($id);
		foreach($user_data as $d) {
			$this->invoices_model->add_custom_field_data(array(
				"invoiceid" => $id,
				"fieldid" => $d['id'],
	...
	public function estimate_status($id, $hash) 
	{
		$id = intval($id);
		$invoice = $this->invoices_model->get_invoice($id);
		if($invoice->num_rows() == 0) {
		...
		if($status == 1) {
			// alert user of acceptance
			$this->user_model->increment_field($invoice->userid, "noti_count", 1);
			$this->user_model->add_notification(array(
				"userid" => $invoice->userid,
				"url" => "invoices/view/" . $id . "/" . $hash,
				"timestamp" => time(),
				"message" => "An invoice estimate has been accepted: " . $invoice->invoice_id,
				"status" => 0,
				"fromid" => $userid,
				"email" => $invoice->acc_email,
				"username" => $invoice->acc_username,
				"email_notification" => $invoice->acc_email_notification
				)
			);
		} else if($status == 2) {
			...
			$this->user_model->add_notification(array(
				...
				"url" => "invoices/view/" . $id . "/" . $hash,
				"timestamp" => time(),
				"message" => "An invoice estimate has been rejected: " . $invoice->invoice_id,
				"status" => 0,
				"fromid" => $userid,
				"email" => $invoice->acc_email,
				"username" => $invoice->acc_username,
	...
	public function client_estimate() 
	{
		$this->check_requirements(array("admin", "project_admin", 
			"invoice_manage", "invoice_client"));
		$this->template->loadData("activeLink", 
			array("invoice" => array("client_estimate" => 1)));
	...
	{
		if(!$this->common->has_permissions(
			array("admin", "project_admin", "invoice_manage"), $this->user
			)
		) {
		...
		$this->template->loadData("activeLink", 
			array("invoice" => array("custom" => 1)));

		$fields = $this->invoices_model->get_custom_fields();

		
		$this->template->loadContent("invoices/custom.php", array(
			"fields" => $fields
			)
		);
	}
	...
	public function add_custom_field() 
	{
		...
		$this->invoices_model->add_custom_field(array(
			"name" => $name,
			"type" => $type,
			"help_text" => $help,
		...
		redirect(site_url("invoices/custom"));
	}
	...
	public function edit_custom_field($id) 
	{
		...
		$id = intval($id);
		$field = $this->invoices_model->get_custom_field($id);
		if($field->num_rows() == 0) {
			$this->template->error(lang("error_214"));
		}

		$this->template->loadContent("invoices/edit_custom_field.php", array(
			"field" => $field->row()
			)
		);
	}
	...
	public function edit_custom_field_pro($id) 
	{
		...
		$this->invoices_model->update_custom_field($id, array(
			"name" => $name,
			"type" => $type,
			"help_text" => $help,
			"select_options" => $select,
	...
	public function delete_custom_field($id, $hash) 
	{
		...
		$this->invoices_model->delete_custom_field($id);
