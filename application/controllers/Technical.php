...
class Technical extends CI_Controller 
{
	...
	public function index($folder_parent = 0, $projectid = -1) 
	{
		$this->template->loadData("activeLink", 
			array("technical" => array("general" => 1)));
		...
		$this->template->loadContent("technical/index.php", array(
			"folders" => $folders,
	...
	public function image($folder_parent = 0, $projectid = -1) 
	{
		...
		$this->template->loadContent("technical/image.php", array(
			"folders" => $folders,
			"folder_parent" => $folder_parent,
			"projectid" => $projectid,
			"projects" => $projects,
	...
	{
		$this->template->loadData("activeLink",array("technical" => array("restore" => 1)));
		...
		$this->template->loadContent("technical/restore.php", array(
			"folders" => $folders,
			"folder_parent" => $folder_parent,
			"projectid" => $projectid,
			"page" => "restore",
	...
	public function file_page1($page, $folder_parent, $projectid = -1) {
		...
		<div class='row'>
			...
			<div class='orderlist1'>
				<img src='/images/addfile.jpg' alt='image' class='image' height='150px' width='150px' data-toggle='modal' data-target='#myModal1' title='Add New Folder/File'>
				</div>
			</a>
		...
		foreach ($rs->result() as $r) {
			...
			$delete_file = '<a href="'.site_url("technical/delete_file/" . $r->ID . "/" . $this->security->get_csrf_hash()).'" class="btn btn-danger btn-sm trash" onclick="return confirm(\''.lang("ctn_508").'\')" title="'.lang("ctn_511").'" data-toggle="tooltip" data-placement="bottom"><span class="glyphicon glyphicon-trash"></span></a>';
			//for deleting folder when we click on delete button on folder
			$delete_folder = '<a href="'.site_url("technical/delete_folder/" . $r->ID . "/" . $this->security->get_csrf_hash()).'" class="btn btn-danger btn-xs trash" onclick="return confirm(\''.lang("ctn_508").'\')" title="'.lang("ctn_509").'" data-toggle="tooltip" data-placement="bottom"><span class="glyphicon glyphicon-trash"></span></a>';
			...
			elseif($r->folder_flag) {
				echo "	
					...
					<div class='row'>
					<div class='col-md-3 col-lg-3'>
					<a class='' href='/index.php/technical/$page/$r->ID/$projectid/'>
					<div class='orderlist1'>
	...
	public function file_restore_page($page, $folder_parent, $projectid = -1) {
		...
		$uri =  $_SERVER["REQUEST_URI"]; //it will print full url
		  ...
		  $id = $uriArray[6];
		//   echo $id;
		$rs = $this->technical_model->get_all_deletedfiles($folder_parent, $projectid);
		$folderArray=array();
		foreach ($rs->result() as $r) {
		...
		foreach ($rs->result() as $r) {
			...
			$restore_file = '<a href="'.site_url("technical/restore_file/" . $r->ID . "/" . $this->security->get_csrf_hash()).'" class="btn btn-primary btn-sm trash" onclick="return confirm(\''.lang("ctn_1622").'\')" title="'.lang("ctn_1623").'" data-toggle="tooltip" data-placement="bottom"><span class="glyphicon glyphicon-retweet"></span></a>';
			//for Restoring folder when we click on restore button on folder
			$restore_folder = '<a href="'.site_url("technical/restore_folder/" . $r->ID . "/" . $this->security->get_csrf_hash()).'" class="btn btn-primary btn-xs trash" onclick="return confirm(\''.lang("ctn_1622").'\')" title="'.lang("ctn_1623").'" data-toggle="tooltip" data-placement="bottom"><span class="glyphicon glyphicon-retweet"></span></a>';
			//if the type as document 
	...
	public function file_page($page, $folder_parent, $projectid = -1) 
	{
		...
		$this->datatables->set_default_order("technical_files.file_name", "asc");

		// Set page ordering options that can be used
		$this->datatables->ordering(
			array(
				 0 => array(
				 	"technical_files.file_name" => 0
				 ),
				 1 => array(
				 	"projects.name" => 0
				 ),
				 2 => array(
				 	"technical_files.file_type" => 0
				 ),
				 3 => array(
				 	"projects.name" => 0
				 )
		...
		if($page == "index") {
			if($projectid > 0) {
				$this->datatables->set_total_rows(
					$this->technical_model->get_files_total($projectid, 
						$folder_parent, $this->user->info->ID)
				);
				$files = $this->technical_model->get_files($projectid, 
					$folder_parent, $this->user->info->ID, $this->datatables);
			} elseif($projectid == 0) {
				$this->datatables->set_total_rows(
					$this->technical_model
					->get_files_user_noproject_total($this->user->info->ID, 
							$folder_parent)
				);
				$files = $this->technical_model
		...
		foreach($files->result() as $r) {
			...
			if($r->folder_flag) {
				...
				$options = '<a href="'.site_url("technical/image/" . $r->ID . "/" . $projectid . "/").'" class="btn btn-primary btn-xs">'.lang("ctn_506").'</a> ';
				if($r->userid == $this->user->info->ID || ($this->common->has_team_permissions(array("admin"), $r)) || ($this->common->has_permissions(array("admin", "project_admin", "file_manage"), $this->user)) ) {
					$options .= '<a href="'.site_url("technical/edit_folder/" . $r->ID).'" class="btn btn-warning btn-xs" title="'.lang("ctn_507").'" data-toggle="tooltip" data-placement="bottom"><span class="glyphicon glyphicon-cog"></span></a> <a href="'.site_url("technical/delete_folder/" . $r->ID . "/" . $this->security->get_csrf_hash()).'" class="btn btn-danger btn-xs" onclick="return confirm(\''.lang("ctn_508").'\')" title="'.lang("ctn_509").'" data-toggle="tooltip" data-placement="bottom"><span class="glyphicon glyphicon-trash"></span></a>';
				}
				...
				$options = '<a href="'.site_url("technical/view_file/" . $r->ID).'" target="_blank" class="btn btn-info btn-xs">'.lang("ctn_493").'</a> ';
				if($r->userid == $this->user->info->ID || ($this->common->has_team_permissions(array("admin"), $r)) || ($this->common->has_permissions(array("admin", "project_admin", "file_manage"), $this->user)) ) {
					$options .= '<a href="'.site_url("technical/edit_file/" . $r->ID).'" class="btn btn-warning btn-xs" data-toggle="tooltip" data-placement="bottom" title="'.lang("ctn_510").'"><span class="glyphicon glyphicon-cog"></span></a> <a href="'.site_url("technical/delete_file/" . $r->ID . "/" . $this->security->get_csrf_hash()).'" class="btn btn-danger btn-xs" onclick="return confirm(\''.lang("ctn_508").'\')" title="'.lang("ctn_511").'" data-toggle="tooltip" data-placement="bottom"><span class="glyphicon glyphicon-trash"></span></a>';
				}
	...
	{
		$folder = $this->technical_model->get_folder($folder_parent);
	...
	public function add_file($folderid = 0) 
	{
		...
		if($folderid > 0) {
			...
			$folder = $this->technical_model->get_folder($folderid);
			if($folder->num_rows() > 0) {
				$folder = $folder->row();
		...
		$this->template->loadContent("technical/add_file.php", array(
			"projects" => $projects,
	...
	public function get_folders_for_project() 
	{
		...
		// Get folders
		$folders = $this->technical_model->get_folders($projectid);
		$this->template->loadAjax("technical/ajax_get_folders.php", array(
			"folders" => $folders,
			"folderid" => $folderid
			),1
		);
	...
	public function add_file_process() 
	{
		...
		if(empty($file_url) && !$folder_flag) 
		{
            ...
            $fileid = $this->technical_model->add_file(array(
            	"file_name" => $name,
            ...
            // File Note
            if(!empty($file_note)) {
            	$this->technical_model->add_file_note(array(
            		"fileid" => $fileid,
            		"userid" => $this->user->info->ID,
            		"note" => $file_note,
            		"timestamp" => time()
		...
		$this->user_model->add_user_log(array(
			...
			. site_url("technical/view_file/" . $fileid) . "'>".lang("ctn_1022")."</a>" ,
			"timestamp" => time(),
		...
		$this->session->set_flashdata("globalmsg", 
			lang("success_42"));
		if($folderid > 0) {
			//previously while add new file it gone to home page i.e., redirect(site_url("technical/index/" . $folderid)), now it will go to current page where we create a folder
			redirect(site_url("technical/image/" . $folderid));
		} else {
			// redirect(site_url("technical"));
			redirect(site_url("technical/all"));
		}
	...
	public function restore_folder($id, $hash) 
	{
		...
		$id = intval($id);
		$file = $this->technical_model->get_file($id);
		if($file->num_rows() == 0) {
	   ...
	   $query = $this->technical_model->get_delete_file($id);
       $deleteFileArray = array();
		...
		$this->technical_model->update_parent_child_files($deleteFileArray,array (
        	"delete_status" => 'No'
        	)
		);
	...
	public function edit_file($id, $all=0) 
	{
		...
		$folders = null;
		if($file->projectid > 0) {
			$folders = $this->technical_model->get_folders($file->projectid);
		}

		$this->template->loadContent("technical/edit_file.php", array(
			"file" => $file,
			"projects" => $projects,
			"folders" => $folders,
			"all" => $all
	...
	public function edit_file_process($id, $all=0) 
	{
		...
		if(empty($file_url) && $_FILES['userfile']['size'] > 0)
		{
            ...
            $this->technical_model->update_file($id, array(
            	"file_name" => $name,
            	"upload_file_name" => $data['file_name'],
            	"projectid" => $projectid,
            	"folder_parent" => $folderid,
		...
		$this->user_model->add_user_log(array(
			...
			. site_url("technical/view_file/" . $id) . "'>".lang("error_22")."</a>" ,
			"timestamp" => time(),
			"IP" => $_SERVER['REMOTE_ADDR'],
			"projectid" => $projectid,
			"url" => "technical"
		...
		if($all == 0) {
			if($folderid > 0) {
				redirect(site_url("technical/index/" . $folderid));
			} else {
				redirect(site_url("technical"));
			}
		} else {
			if($folderid > 0) {
				redirect(site_url("technical/all/" . $folderid));
			} else {
				redirect(site_url("technical/all"));
			}
		}
	...
	public function edit_folder($id, $all=0) 
	{
		...
		$this->template->loadContent("technical/edit_folder.php", array(
			"file" => $file,
			"projects" => $projects,
			"folders" => $folders,
			"all" => $all
	...
	public function edit_folder_process($id, $all) 
	{
		...
		$this->user_model->add_user_log(array(
			...
			. site_url("technical/view_file/" . $id) . "'>".lang("ctn_1022")."</a>" ,
			"timestamp" => time(),
			"IP" => $_SERVER['REMOTE_ADDR'],
			"projectid" => $projectid,
			"url" => "technical"
	...
	public function delete_file($id, $hash) 
	{
		...
		*/
		...
		// $this->technical_model->delete_file($id);
		...
		// echo $var;
		// exit;
		$this->technical_model->update_parent_child_files($id,array(
        	"delete_status" => 'Yes'
        	)
		);
	...
	public function view_file($id) 
	{
		...
		$notes = $this->technical_model->get_file_notes($id);

		$this->template->loadContent("technical/view_file.php", array(
			"file" => $file,
			"notes" => $notes,
	...
	public function delete_file_note($id, $hash) 
	{
		...
		$id = intval($id);
		$note = $this->technical_model->get_file_note($id);
		if($note->num_rows() == 0) {
			$this->template->error(lang("error_101"));
		}
		$note = $note->row();

		$this->technical_model->delete_file_note($id);
		$projectid = 0;

		// Get user permission
		if($note->userid != $this->user->info->ID) {
			$file = $this->technical_model->get_file($note->fileid);
			if($file->num_rows() == 0) {
				$this->template->error(lang("error_101"));
			}
			$file = $file->row();
		...
		$this->user_model->add_user_log(array(
			...
			"message" => lang("ctn_1027") . " <a href='". 
				site_url("technical/view_file/" . $note->fileid) ."'>".lang("ctn_1022")."</a>" ,
			"timestamp" => time(),
			"IP" => $_SERVER['REMOTE_ADDR'],
			"projectid" => $projectid,
			"url" => "technical"
		...
		$this->session->set_flashdata("globalmsg", 
			lang("success_46"));
		redirect(site_url("technical/view_file/" . $note->fileid));
	}
	...
	public function add_file_note($id) 
	{
		...
		$this->session->set_flashdata("globalmsg", 
			lang("success_47"));
		redirect(site_url("technical/view_file/" . $id));
	}
}
