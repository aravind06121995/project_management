...
class File_Model extends CI_Model 
{
	...
	public function get_all_files1($folder_parent, $projectid) 
	{
		...
		return $this->db
			...
			->group_start()
			->where("project_files.folder_parent", $folder_parent)
			->where("project_files.delete_status = 'No'")
			->where("(projects.status = 0 OR projects.status IS NULL)")
			->group_end()
			->order_by("project_files.ID", "DESC")
			->get("project_files");
	...
	public function get_delete_file($id) {
		...
		return
 		$query= $this->db->query("(SELECT * FROM project_files WHERE ID=$id) UNION 
		 (SELECT * from project_files where FIND_IN_SET(folder_parent,@pv) and @pv:= concat(@pv, ',',ID))");
	}
	
	public function update_parent_child_files($id,$data) 
	{
	
		$this->db->where_in("ID",$id)->update("project_files", $data);
